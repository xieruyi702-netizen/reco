...
## 防作弊评测协议与级联实验的最终结论（2026-09-05）

为避免"在评测集上反复调参过拟合"，建立 dev/test 双拆分协议：
**test 集 = seed7 分层 20 题（S4 基线已冻结），dev 集 = seed13 分层 20 题（零交集）**；
改进只在 dev 上调优与选型，选定后在 test 上**只跑一次**定稿，不回头。

### 候选改进（全部零 gold 依赖，只用自身工具证据）

- **计数自洽校验**：合成答案为纯数字、且轨迹中恰有一个 COUNT 聚合值与之不符 → 判定
  模型眼数出错，校正为聚合值
- **置信升级级联**：答案含 insufficient / 有空结果步骤 → 升级为全工具 ReAct 兜底

### 结果

| 配置 | dev agg/answer | test agg/answer | test tok |
|---|---|---|---|
| 纯 PE（v2） | 0.626 / 0.35 | 0.602 / 0.40 | 1173 |
| PE+校验+级联 | **0.656 / 0.45** | 0.570 / 0.35 | 3022 |
| S4 基线（参照） | — | 0.695 / 0.65 | 4211 |

**结论：dev 上的 +10pp answer 在 test 上未复现（-5pp），n=20 下该差异与噪声不可区分。
级联改进被协议拦下，未定稿。** 这正是双拆分协议的价值：它阻止了一次对 20 题样本的
过拟合宣称。也给出诚实的工程边界：在 20 题样本量下，能可靠分辨的改进幅度 ≥10pp；
要验证更细粒度的改进，需扩样本至 100 题（成本约 5 倍）。

### 当前最终定稿配置

- 精度优先：**S4 ReAct-all**（agg 0.695 / answer 0.65 / 4211 tok）
- 效率优先：**S7 Plan-and-Execute v2**（agg 0.602 / answer 0.40 / 1173 tok，evidence 0.725 反超 S4）
